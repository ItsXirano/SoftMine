...
class CrashDump{
	...
	public function __construct(Server $server){
		$this->time = time();
		$this->server = $server;
		$this->path = $this->server->getDataPath() . "/crashs/CrashDump_" . date("D_M_j-H.i.s-T_Y", $this->time) . ".log";
	...
	private function pluginsData(){
		if(class_exists("softmine\\plugin\\PluginManager", false)){
			$this->addLine();
	...
	private function extraData(){
		...
		if($this->server->getProperty("auto-report.send-settings", true) !== false){
			...
			$this->data["softmine.yml"] = @file_get_contents($this->server->getDataPath() . "softmine.yml");
		}else{
			$this->data["softmine.yml"] = "";
	...
	private function baseCrash(){
		...
		if(strpos($error["file"], "src/softmine/") === false and strpos($error["file"], "src/raklib/") === false and file_exists($error["fullFile"])){
			$this->addLine();
			...
			foreach($this->server->getPluginManager()->getPlugins() as $plugin){
				$filePath = \softmine\cleanPath($file->getValue($plugin));
	...
	private function generalData(){
		...
		$this->data["general"]["api"] = \softmine\API_VERSION;
		$this->data["general"]["git"] = \softmine\GIT_COMMIT;
		...
		$this->addLine("softmine version: " . $version->get(false) . " #" . $version->getBuild() . " [Protocol " . Info::CURRENT_PROTOCOL . "; API " . API_VERSION . "]");
