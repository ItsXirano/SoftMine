...
use softmine\command\CommandReader;
use softmine\command\CommandSender;
use softmine\command\ConsoleCommandSender;
use softmine\command\PluginIdentifiableCommand;
use softmine\command\SimpleCommandMap;
use softmine\entity\Arrow;
use softmine\entity\Attribute;
use softmine\entity\Effect;
use softmine\entity\Entity;
use softmine\entity\FallingSand;
use softmine\entity\Human;
use softmine\entity\Item as DroppedItem;
use softmine\entity\PrimedTNT;
use softmine\entity\Snowball;
use softmine\entity\Squid;
use softmine\entity\Villager;
use softmine\entity\Zombie;
use softmine\event\HandlerList;
use softmine\event\level\LevelInitEvent;
use softmine\event\level\LevelLoadEvent;
use softmine\event\server\QueryRegenerateEvent;
use softmine\event\server\ServerCommandEvent;
use softmine\event\Timings;
use softmine\event\TimingsHandler;
use softmine\event\TranslationContainer;
use softmine\inventory\CraftingManager;
use softmine\inventory\InventoryType;
use softmine\inventory\Recipe;
use softmine\inventory\ShapedRecipe;
use softmine\inventory\ShapelessRecipe;
use softmine\item\enchantment\Enchantment;
use softmine\item\Item;
use softmine\lang\BaseLang;
use softmine\level\format\anvil\Anvil;
use softmine\level\format\leveldb\LevelDB;
use softmine\level\format\LevelProviderManager;
use softmine\level\format\mcregion\McRegion;
use softmine\level\generator\biome\Biome;
use softmine\level\generator\Flat;
use softmine\level\generator\Generator;
use softmine\level\generator\hell\Nether;
use softmine\level\generator\normal\Normal;
use softmine\level\Level;
use softmine\metadata\EntityMetadataStore;
use softmine\metadata\LevelMetadataStore;
use softmine\metadata\PlayerMetadataStore;
use softmine\nbt\NBT;
use softmine\nbt\tag\ByteTag;
use softmine\nbt\tag\CompoundTag;
use softmine\nbt\tag\DoubleTag;
...
use softmine\network\upnp\UPnP;
use softmine\permission\BanList;
use softmine\permission\DefaultPermissions;
use softmine\plugin\PharPluginLoader;
use softmine\plugin\Plugin;
use softmine\plugin\PluginLoadOrder;
use softmine\plugin\PluginManager;
use softmine\plugin\ScriptPluginLoader;
use softmine\scheduler\FileWriteTask;
use softmine\scheduler\SendUsageTask;
use softmine\scheduler\ServerScheduler;
use softmine\tile\Chest;
use softmine\tile\EnchantTable;
use softmine\tile\Furnace;
use softmine\tile\Sign;
use softmine\tile\Tile;
use softmine\updater\AutoUpdater;
use softmine\utils\Binary;
use softmine\utils\Config;
use softmine\utils\LevelException;
use softmine\utils\MainLogger;
use softmine\utils\ServerException;
use softmine\utils\ServerKiller;
use softmine\utils\Terminal;
use softmine\utils\TextFormat;
use softmine\utils\Utils;
use softmine\utils\UUID;
use softmine\utils\VersionString;
...
class Server{
	...
	public function getVersion(){
		return \softmine\MINECRAFT_VERSION;
	}
	...
	public function getOfflinePlayerData($name){
		...
		if(file_exists($path . "$name.dat")){
			...
			$this->logger->notice($this->getLanguage()->translateString("softmine.data.playerNotFound", [$name]));
		}
		...
		if(file_exists($path . "$name.yml")){ //Importing old softmine files
			...
			$this->logger->notice($this->getLanguage()->translateString("softmine.data.playerOld", [$name]));
	...
	public function saveOfflinePlayerData($name, CompoundTag $nbtTag, $async = false){
		...
		try{
			...
			$this->logger->critical($this->getLanguage()->translateString("softmine.data.saveError", [$name, $e->getMessage()]));
	...
	public function loadLevel($name){
		...
		if($this->isLevelLoaded($name)){
			...
			$this->logger->notice($this->getLanguage()->translateString("softmine.level.notFound", [$name]));
		...
		if($provider === null){
			$this->logger->error($this->getLanguage()->translateString("softmine.level.loadError", [$name, "Unknown provider"]));
		...
		try{
			...
			$this->logger->error($this->getLanguage()->translateString("softmine.level.loadError", [$name, $e->getMessage()]));
			$this->logger->logException($e);
			return false;
		}
	 ...
	 * @param string $name
	 * @param int    $seed
	 * @param string $generator Class name that extends softmine\level\generator\Noise
	 * @param array  $options
	 *
	 * @return bool
	 */
	public function generateLevel($name, $seed = null, $generator = null, $options = []){
		...
		try{
			$path = $this->getDataPath() . "worlds/" . $name . "/";
			/** @var \softmine\level\format\LevelProvider $provider */
			...
			$this->logger->error($this->getLanguage()->translateString("softmine.level.generateError", [$name, $e->getMessage()]));
			$this->logger->logException($e);
			return false;
		}
		...
		$this->getLogger()->notice($this->getLanguage()->translateString("softmine.level.backgroundGeneration", [$name]));
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath){
		...
		try{
			...
			$this->logger->info("Loading softmine.yml...");
			if(!file_exists($this->dataPath . "softmine.yml")){
				$content = file_get_contents($this->filePath . "src/softmine/resources/softmine.yml");
				if($version->isDev()){
					$content = str_replace("preferred-channel: stable", "preferred-channel: beta", $content);
				}
				@file_put_contents($this->dataPath . "softmine.yml", $content);
			}
			$this->config = new Config($this->dataPath . "softmine.yml", Config::YAML, []);
			...
			$this->logger->info($this->getLanguage()->translateString("softmine.server.start", [TextFormat::AQUA . $this->getVersion()]));
			...
			define('softmine\DEBUG', (int) $this->getProperty("debug.level", 1));

			ini_set('assert.exception', 1);

			if($this->logger instanceof MainLogger){
				$this->logger->setLogDebug(\softmine\DEBUG > 1);
			}
			...
			$this->logger->info($this->getLanguage()->translateString("softmine.server.networkStart", [$this->getIp() === "" ? "*" : $this->getIp(), $this->getPort()]));
			...
			$this->logger->info($this->getLanguage()->translateString("softmine.server.info", [
				$this->getName(),
				($version->isDev() ? TextFormat::YELLOW : "") . $version->get(true) . TextFormat::WHITE,
				$this->getCodename(),
				$this->getApiVersion()
			]));
			$this->logger->info($this->getLanguage()->translateString("softmine.server.license", [$this->getName()]));
			...
			$this->updater = new AutoUpdater($this, $this->getProperty("auto-updater.host", "https://github.com/PocketSoftMine/SoftMine"));
			...
			if(extension_loaded("leveldb")){
				$this->logger->debug($this->getLanguage()->translateString("softmine.debug.enable"));
			...
			if(!($this->getDefaultLevel() instanceof Level)){
				$this->getLogger()->emergency($this->getLanguage()->translateString("softmine.level.defaultError"));
				$this->forceShutdown();
	...
	/**
	 * Starts the softmine server and starts processing ticks and packets
	 */
	public function start(){
		...
		$this->logger->info($this->getLanguage()->translateString("softmine.server.defaultGameMode", [self::getGamemodeString($this->getGamemode())]));

		$this->logger->info($this->getLanguage()->translateString("softmine.server.startFinished", [round(microtime(true) - \softmine\START_TIME, 3)]));
	...
	public function crashDump(){
		...
		$this->logger->emergency($this->getLanguage()->translateString("softmine.crash.create"));
		try{
			$dump = new CrashDump($this);
		}catch(\Throwable $e){
			$this->logger->critical($this->getLanguage()->translateString("softmine.crash.error", $e->getMessage()));
			return;
		}
		...
		if($this->getProperty("auto-report.enabled", true) !== false){
			...
			if($report){
				$reply = Utils::postURL("http://" . $this->getProperty("auto-report.host", "crash.softmine.net") . "/submit/api", [
					"report" => "yes",
					"name" => $this->getName() . " " . $this->getPocketMineVersion(),
					"email" => "crash@softmine.net",
				...
				if(($data = json_decode($reply)) !== false and isset($data->crashId)){
					...
					$this->logger->emergency($this->getLanguage()->translateString("softmine.crash.archive", [$reportUrl, $reportId]));
				}
			}
		}
	...
	private function checkTickUpdates($currentTick, $tickTime){
		...
		foreach($this->getLevels() as $level){
			...
			try{
				...
				$this->logger->critical($this->getLanguage()->translateString("softmine.level.tickError", [$level->getName(), $e->getMessage()]));
				$this->logger->logException($e);
			}
		}
	}
	...
	private function tick(){
		...
		if(($this->tickCounter % 100) === 0){
			...
			if($this->getTicksPerSecondAverage() < 12){
				$this->logger->warning($this->getLanguage()->translateString("softmine.server.tickOverload"));
			}
		}
