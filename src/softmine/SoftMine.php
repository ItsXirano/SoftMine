...
namespace softmine {
	...
	if(!class_exists("ClassLoader", false)){
		require_once(\softmine\PATH . "src/spl/ClassLoader.php");
		require_once(\softmine\PATH . "src/spl/BaseClassLoader.php");
		require_once(\softmine\PATH . "src/softmine/CompatibleClassLoader.php");
	}

	$autoloader = new CompatibleClassLoader();
	$autoloader->addPath(\softmine\PATH . "src");
	$autoloader->addPath(\softmine\PATH . "src" . DIRECTORY_SEPARATOR . "spl");
	...
	define('softmine\START_TIME', microtime(true));

	$opts = getopt("", ["data:", "plugins:", "no-wizard", "enable-profiler"]);

	define('softmine\DATA', isset($opts["data"]) ? $opts["data"] . DIRECTORY_SEPARATOR : \getcwd() . DIRECTORY_SEPARATOR);
	define('softmine\PLUGIN_PATH', isset($opts["plugins"]) ? $opts["plugins"] . DIRECTORY_SEPARATOR : \getcwd() . DIRECTORY_SEPARATOR . "plugins" . DIRECTORY_SEPARATOR);

	Terminal::init();

	define('softmine\ANSI', Terminal::hasFormattingCodes());

	if(!file_exists(\softmine\DATA)){
		mkdir(\softmine\DATA, 0777, true);
	}

	//Logger has a dependency on timezone, so we'll set it to UTC until we can get the actual timezone.
	date_default_timezone_set("UTC");

	$logger = new MainLogger(\softmine\DATA . "server.log", \softmine\ANSI);
	...
	function cleanPath($path){
		return rtrim(str_replace(["\\", ".php", "phar://", rtrim(str_replace(["\\", "phar://"], ["/", ""], \softmine\PATH), "/"), rtrim(str_replace(["\\", "phar://"], ["/", ""], \softmine\PLUGIN_PATH), "/")], ["/", "", "", "", ""], $path), "/");
	}

	$errors = 0;

	if(php_sapi_name() !== "cli"){
		$logger->critical("You must run softmine using the CLI.");
		++$errors;
	}
	...
	if(extension_loaded("softmine")){
		if(version_compare(phpversion("softmine"), "0.0.1") < 0){
			$logger->critical("You have the native softmine extension, but your version is lower than 0.0.1.");
			++$errors;
		}elseif(version_compare(phpversion("softmine"), "0.0.4") > 0){
			$logger->critical("You have the native softmine extension, but your version is higher than 0.0.4.");
			++$errors;
		}
	}
	...
	if(!file_exists(\softmine\DATA . "server.properties") and !isset($opts["no-wizard"])){
		new Installer();
	}

	if(\Phar::running(true) === ""){
		$logger->warning("Non-packaged softmine installation detected, do not use on production.");
	}

	ThreadManager::init();
	$server = new Server($autoloader, $logger, \softmine\PATH, \softmine\DATA, \softmine\PLUGIN_PATH);
